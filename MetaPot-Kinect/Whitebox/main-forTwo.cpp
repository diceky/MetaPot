...
int _tmain( int argc, _TCHAR* argv[] )
{
	...
	char time_buff[50] = "";
	char outfileAudio[50] = "";
	char outfileBody[50] = "";
	char outfileFace[50] = "";
	time_t now = time(NULL);
	struct tm pnow;
	localtime_s(&pnow, &now);
	sprintf_s(time_buff, 50, "%04d%02d%02d_%02d%02d%02d", pnow.tm_year + 1900, pnow.tm_mon + 1, pnow.tm_mday, pnow.tm_hour, pnow.tm_min, pnow.tm_sec);
	std::cout << time_buff << std::endl;

	//MAKE DIRECTORY WITH NAME AS DATE TIME
	wchar_t wtext[20];
	size_t ret;
	mbstowcs_s(&ret, wtext, 20, time_buff, strlen(time_buff)+1);//Plus null
	LPWSTR mkdir_ptr = wtext;
	CreateDirectory(mkdir_ptr, NULL);

	//MAKE OUTPUT FILE UNDER THE NEW DIRECTORY
	sprintf_s(outfileAudio, 50, "./%s/audio.csv", time_buff);
	sprintf_s(outfileBody, 50, "./%s/body.csv", time_buff);
	sprintf_s(outfileFace, 50, "./%s/face.csv", time_buff);
	std::cout << outfileAudio << ", " << outfileBody << ", " << outfileFace << std::endl;
	std::ofstream outputfileAudio(outfileAudio);
	std::ofstream outputfileBody(outfileBody);
	std::ofstream outputfileFace(outfileFace);
	...
	int myIndex = 0;//for identifying right or left
			...
			if(SUCCEEDED(hResult)){
				for (int y = 0; y<height; y++){
					for (int x = 0; x < width; x++){
						...
						if (buffer[index] != 0xff && x < 256){
							bodyIndexMat.at<cv::Vec3b>(y, x) = color[2];//color in RED
						}
						else if (buffer[index] != 0xff && x >= 256){
							bodyIndexMat.at<cv::Vec3b>(y, x) = color[1];//color in GREEEN
						}
						else{
			...
			if(SUCCEEDED(hResult)){
				...
				for (count = 0; count < BODY_COUNT; count++){
					...
					if(SUCCEEDED(hResult) && bTracked){
						...
						//GET HIP COORDINATES FOR IDENTIFYING PERSON
						ColorSpacePoint colorSpacePoint = { 0 };
						pCoordinateMapper->MapCameraPointToColorSpace( joint[0].Position, &colorSpacePoint );
						int hip_x = static_cast<int>( colorSpacePoint.X );
						int hip_y = static_cast<int>( colorSpacePoint.Y );
						if(hip_x < 860){
							myIndex=2;
						}
						else if(hip_x >= 860){
							...
							if(SUCCEEDED(hResult)){
								...
								if(SUCCEEDED(hResult)){
									...
									*/
									...
									if(myIndex==2){
										std::cout << "A ";//left side of screen: A
										outputfileBody << "A,";//left side of screen: A
									}
									else if(myIndex==1){
										outputfileBody << "B,";//right side of screen: B
										std::cout << "B ";//right side of screen: B
									}
									outputfileBody << x << "," << y;
									std::cout << "LeftHandX : " << x << ", LeftHandY : " << y;
									cv::circle(handcoord_img, cv::Point(x*930/1920, y*510/1080), 20, color_opencv[myIndex], -1, CV_AA);
								}
							}
							...
							if(SUCCEEDED(hResult)){
								...
								if(SUCCEEDED(hResult)){
									...
									*/
									...
									outputfileBody << "," << x << "," << y;
									std::cout << "RightHandX : " << x << ", RightHandY : " << y << std::endl;
									cv::circle(handcoord_img, cv::Point(x*930/1920, y*510/1080), 20, color_opencv[myIndex], -1, CV_AA);
								}
							}
				...
				if( SUCCEEDED( hResult ) && bFaceTracked ){
					...
					if( SUCCEEDED( hResult ) && pFaceResult != nullptr ){
						...
						if( SUCCEEDED( hResult ) ){
							...
							outputfileFace << "," << static_cast<int>( facePoint[2].X ) << "," << static_cast<int>( facePoint[2].Y );
							cv::circle( bufferMat_face, cv::Point( static_cast<int>( facePoint[3].X ), static_cast<int>( facePoint[3].Y ) ), 5, static_cast<cv::Scalar>( color[myIndex] ), -1, CV_AA ); // Mouth (Left)
							outputfileFace << "," << static_cast<int>( facePoint[3].X ) << "," << static_cast<int>( facePoint[3].Y );
							cv::circle( bufferMat_face, cv::Point( static_cast<int>( facePoint[4].X ), static_cast<int>( facePoint[4].Y ) ), 5, static_cast<cv::Scalar>( color[myIndex] ), -1, CV_AA ); // Mouth (Right)
							outputfileFace << "," << static_cast<int>( facePoint[4].X ) << "," << static_cast<int>( facePoint[4].Y );
						}
						...
						if( SUCCEEDED( hResult ) ){
							...
							outputfileFace << "," << std::to_string( pitch ) << "," << std::to_string( yaw ) << "," << std::to_string( roll );
						}
	...
	outputfileAudio.close();
	outputfileBody.close();
	outputfileFace.close();
